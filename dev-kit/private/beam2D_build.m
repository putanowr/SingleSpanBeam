...
reporoot = beam2D.globals.distroot;
origDir = pwd;
RAII.cd = onCleanup(@() cd(origDir));
cd(reporoot)

fprintf('Building SingleSpanBeam...\n')

cfgFile = fullfile(reporoot, '.mlproject.json');
config = jsondecode(beam2D.internal.util.readtext(cfgFile));
