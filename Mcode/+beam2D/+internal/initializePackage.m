...
% that's done in beam2D.Settings.discover. It has to be that way so the package
% settings state can handle a `clear classes` gracefully.

% Do not re-initialize if already initialized
if beam2D.internal.misc.getpackageappdata('initialized')
  return
end

% Don't depend on globals, to avoid circular dependency
% distroot = beam2D.globals.distroot;
...
beam2D.internal.misc.setpackageappdata('initialized', true);
