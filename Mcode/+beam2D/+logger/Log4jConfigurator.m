classdef Log4jConfigurator
    ...
    % Examples:
    %
    % beam2D.logger.Log4jConfigurator.configureBasicConsoleLogging
    %
    % beam2D.logger.Log4jConfigurator.setLevels({'root','DEBUG'});
    %
    % beam2D.logger.Log4jConfigurator.setLevels({
    %     'root'    'INFO'
    %     'net.apjanke.logger.swing'  'DEBUG'
    %     });
    %
    % beam2D.logger.Log4jConfigurator.prettyPrintLogConfiguration
    %
    % % Display fully-qualified class/category names in the log output:
    % beam2D.logger.Log4jConfigurator.setRootAppenderPattern(...
    %    ['%d{HH:mm:ss.SSS} %p %c - %m' sprintf('\n')]);
    %
    % % Bring up the configuration GUI
    % beam2D.logger.Log4jConfigurator.showGui
    ...
    methods (Static)
        ...
        function setLevels(levels)
            ...
            %
            % beam2D.logger.Log4jConfigurator.setLevels(levels)
            %
            ...
            for i = 1:size(levels, 1)
                ...
                level = beam2D.logger.Log4jConfigurator.getLog4jLevel(levelName);
