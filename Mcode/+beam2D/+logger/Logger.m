classdef Logger
  ...
  % arguments.
  %    beam2D.logger.info(format, varargin)
  ...
  % message.
  %    beam2D.logger.warn(exception, format, varargin)
  %
  % See also:
  % beam2D.logger.error
  % beam2D.logger.warn
  ...
  % Examples:
  %
  % log = beam2D.logger.Logger.getLogger('foo.bar.FooBar');
  ...
  methods (Static)
    function out = getLogger(identifier)
      ...
      out = beam2D.logger.Logger(jLogger);
    end
  end
